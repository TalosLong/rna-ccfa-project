...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Legacy NMR collection | `/root/autodl-tmp/data/sequences`; `/root/autodl-tmp/data/ss/gt`; `/root/autodl-tmp/data/structures` | **PARTIAL** — the roots have no source README, but the Legacy121 v1 evaluation mapping is frozen in `manifests/legacy121_v1.csv`. | 121 primary one-record FASTAs; 123 GT `.db`; 121 rows in Legacy121 v1; 147 CIF in the structure pool. | One-record FASTA; `all.fasta` is a 121-record aggregate. The ligand/protein aggregate FASTAs are separate complex assets. | One-line extended dot-bracket in `ss/gt`; filenames end in `_matrix.db`. | **YES** — 13/123 GT files encode crossing bracket families; 11 occur inside Legacy121 v1. | **YES, 3D mapping not frozen** — matching CIFs exist by ID prefix, but explicit chain/model mapping is not documented. | Frozen Legacy121 v1 Phase 0 2D universe on the 121 primary-sequence intersection. |
...
| --- | --- | --- |
| Legacy NMR GT | `/root/autodl-tmp/data/ss/gt/*.db` | One-line extended dot-bracket. The 121-row Legacy121 v1 mapping is frozen in `manifests/legacy121_v1.csv`; the two documented GT-only records remain outside that version. |
...
1. **Resolved for Legacy121 v1:** the universe is the 121 primary FASTA intersection in `manifests/legacy121_v1.csv`. The preserved GT-only records `8Q4O_23_g4_nmr_matrix.db` and `8TNS_24_g4_nmr_matrix.db` are explicitly excluded; the mapping and PETfold alias are audited in `docs/legacy121_v1_protocol.md`.
...
Counts were produced with read-only filesystem enumeration and CSV/manifest inspection on 2026-08-25. Legacy121 v1 mapping and canonical-parser validation were audited read-only on 2026-08-26. Dot-bracket crossing counts require a non-parenthesis bracket family. The TS85 CASP18 count was obtained directly from pair lists using the crossing criterion `i < k < j < l` (or its symmetric ordering). These are representation-level checks and do not independently assert a biological pseudoknot classification.
