...
| Preserving the source predictor output adds value over refolding from sequence under the same evidence. | B2 removed 32,433 original FP and added 11,042 new TP, but also lost 4,752 original TP and added 10,823 new FP; Micro TP preservation was 0.981767. This establishes preservation headroom but does not show that a post-hoc method can exploit it. | **PLAUSIBLE HEADROOM / NOT YET TESTED BY R4** | Gate A only after a future prospectively frozen R4 is compared against B2 on the frozen correction-preservation trade-off. |
