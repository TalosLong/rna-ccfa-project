...
| Global constrained refolding is a strong alternative to post-hoc reconciliation. | Established prior-art mechanism; not yet evaluated in this benchmark under matched evidence. | **MANDATORY BASELINE NOT YET TESTED** | R2 matched-evidence Legacy121 evaluation. |
| Preserving the source predictor output adds value over refolding from sequence under the same evidence. | None under matched B2 comparison. | **PRIMARY CLAIM NOT TESTED** | Gate A in R2/R4; compare TP-preservation/FP-removal risk–utility trade-off. |
...
| Evidence can improve non-evidenced pairs without excessive collateral TP loss. | B1 cannot test propagation; no B2/R4 matched comparison yet. | **NOT TESTED** | R2 and R4 non-evidenced modification precision, FP removal and TP loss. |
