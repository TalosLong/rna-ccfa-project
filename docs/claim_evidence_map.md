...
| H3: exact cross-model consensus can act as a safety veto for a learned deletion score. | Retrospective authoritative BASE audit: support=2 contains 211/282 harmful edit events but only 18/1,571 beneficial events. | MECHANISTICALLY_SUPPORTED_HYPOTHESIS | This motivates the frozen v3 protocol only. No v3 condition has been evaluated, and Legacy121 is not independent evidence. |
...
| Learned refinement generalizes across datasets. | external77 GT_CON candidate membership, all three 42/42 source conditions, and the 126-record normalized matrix are frozen and validated. | NOT_TESTED | Both learned Legacy121 gates failed; v3 is not yet evaluated; external77 was deliberately not evaluated. |
