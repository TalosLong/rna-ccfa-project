...
| Observable pair/topology features support a learned selective refiner. | No learned model has been implemented or trained; only a leakage-safe protocol and Legacy121 label inventory are frozen. | NOT_TESTED | Requires grouped development experiments and a locked independent external test. |
...
| Learned refinement generalizes across datasets. | external77 GT_CON candidate membership has been independently reproduced and its 42-row manifest frozen; RNAfold and query-only trRosettaRNA2 native-SS sources are valid 42/42, while PETfold historical alignment/projection remains blocked and no normalized three-source matrix exists. | NOT_TESTED | The candidate is test-ready only after all 126 source records, source protocols, and normalization pass validation. |
