...
| Learned refinement generalizes across datasets. | external77 GT_CON candidate membership has been independently reproduced and its 42-row manifest frozen; only RNAfold is currently complete (42/42), while PETfold/trRosettaRNA2 are blocked and no normalized three-source matrix exists. | NOT_TESTED | The candidate is test-ready only after all 126 source records, source protocols, and normalization pass validation. |
