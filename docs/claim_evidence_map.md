...
| Claim B: cross-model agreement provides transferable correction evidence. | Prediction-only audit was promising; v2 trained 50 CROSS backbones, but the primary gate failed (8/25 deployable thresholds, recall and matched-effect failures). | PARTIALLY_SUPPORTED | Cross features support high-precision/high-preservation Legacy121 deletions, but not the preregistered improvement claim; no external evidence exists. |
